...
if (process.env.DATABASE_URL.startsWith("file:")) {
  ...
  if (!fs.existsSync(databasePath)) {
    console.log(`SQLite database not found at ${databasePath}; treating as fresh bootstrap and skipping drift check.`);
    process.exit(0);
  }
}

const statusResult = spawnSync(process.execPath, [path.join("scripts", "prisma-run.js"), "migrate", "status"], {
  cwd: repoRoot,
  env: process.env,
  encoding: "utf8",
});
